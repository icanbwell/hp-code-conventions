...
| `404 Not Found` fetching `registry.npmjs.org/@rootio%2f...` | #6 setup-node clobbered the JFrog config | [#6](references/failure-modes.md#6-actions-setup-node-registry-url-silently-breaks-npmrc-writes) |
| `401 Unauthorized` or `403 Forbidden` fetching `npm.pkg.github.com/download/@icanbwell/...` | #5 (401, no token) or #7 (403, wrong token) | [#5](references/failure-modes.md#5-jfrogs-virtual-registry-passes-through-origin-urls-unchanged), [#7](references/failure-modes.md#7-github_token-package-read-access-is-granted-per-package) |
